...
import classes from './AdminDelUserPage.css';
...
class AdminDelUserPage extends React.Component{
    constructor(props){
        super(props);
        this.state={
            tasks:[],
    ...
    componentDidMount(){
        fetch("http://91.231.86.36/admin/usertasks/get/")
    ...
    render(){
        const handleDecline = (user) => {
            fetch("http://91.231.86.36/admin/authordelete/decline",{
                method:"POST",
                headers:{
        ...
        const handleAccept = (user) => {
            fetch("http://91.231.86.36/admin/authordelete/accept",{
                method:"POST",
                headers:{
                    "Content-Type":"application/json"
                },
            ...
            <AdminContainer>
                <Grid container item xs={9} direction={"column"} justify={"center"} className={classes.Table}>
                    <Grid container justify={'center'} item xs>
                        <h2>Список заявок на удаление аккаунта</h2>
                    </Grid>
                    <Grid item xs>
                        <Table>
                            <TableHead>
...
export default AdminDelUserPage;